...
public class Test {
    ...
    public void Read(ArraySegment<byte> segment) {
        ...
        this.testId = BitConverter.ToInt64(s.Slice(count, s.Length - count));
		count += sizeof(long);
		ushort nameLen = BitConverter.ToUInt16(s.Slice(count, s.Length - count));
		count += sizeof(ushort);
		this.name = Encoding.Unicode.GetString(s.Slice(count, nameLen));
		count += nameLen;
		this.testFloat = BitConverter.ToSingle(s.Slice(count, s.Length - count));
		count += sizeof(float);
    ...
    public ArraySegment<byte> Write() {
		...
		count += sizeof(ushort);
		count += nameLen;
		success &= BitConverter.TryWriteBytes(s.Slice(count, s.Length - count), this.testFloat);
		count += sizeof(float);
