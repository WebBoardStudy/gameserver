...
public class Listener {
    ...
    private void OnAcceptCompleted(object? sender, SocketAsyncEventArgs args) {
        if (args.SocketError == SocketError.Success) {
            try {
                Session? newSession = (_sessionFactory?.Invoke()) ?? throw new NullReferenceException("Session Factory output null");
                newSession.Start(args.AcceptSocket ?? throw new NullReferenceException("args.AcceptSocket is null"));
                newSession.OnConnectd(args.AcceptSocket.RemoteEndPoint ?? throw new NullReferenceException("args.AcceptSocket.RemoteEndPoint is null"));
            } catch (Exception ex) {
